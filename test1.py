...
def dashboard_page():
    ...
    if st.session_state.csv_data is not None:
        ...
        with col1:
            st.info(f"ℹ Dataset: *{len(st.session_state.csv_data):,}* baris")
        with col2:
            st.info(f"📋 Columns: *{len(st.session_state.csv_data.columns)}*")
    else:
        st.warning("⚠ Belum ada data. Upload file CSV di menu 'Add Data'")
...
def display_data_page():
    ...
    if st.session_state.csv_data is not None:
        ...
        st.info(f"ℹ Total data: *{total_rows:,}* baris (UNLIMITED CAPACITY)")
        ...
        st.warning("⚠ Tidak ada data. Upload file CSV di menu 'Add Data'")
...
def cleaning_data_page():
    ...
    if st.session_state.csv_data is not None:
        total_rows = len(st.session_state.csv_data)
        st.info(f"ℹ Data saat ini: *{total_rows:,}* baris")
        ...
        st.warning("⚠ Tidak ada data untuk dibersihkan. Upload file CSV terlebih dahulu.")
                ...
                try:
                    with st.spinner("Loading data..."):
                        ...
                        if df is not None:
                            ...
                            with st.expander("👁 Preview (10 baris)"):
...
def delete_data_page():
    ...
    if st.session_state.csv_data is not None:
        total_rows = len(st.session_state.csv_data)
        st.warning("⚠ Penghapusan data bersifat permanen!")
        st.info(f"ℹ Total: *{total_rows:,}* baris")
        ...
        if st.button("🗑 Hapus Range", type="primary"):
...
def generate_forecast_page():
    ...
    if data_source is None:
        st.warning("⚠ Tidak ada data. Upload dan clean data terlebih dahulu.")
        return
    
    st.success(f"✅ Data tersedia: {len(data_source):,} baris")
    
    # Forecast Configuration
    st.subheader("⚙ Konfigurasi Forecast")
