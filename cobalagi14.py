...
def log_upload_history(user_id, username, filename, file_size_mb, total_rows, status='success', error_msg=None):
    """
    Log CSV upload to history table - FIXED with better error handling
    """
    conn = None
    try:
        st.info("🔌 Connecting to database for upload history...")
        conn = get_connection()
        ...
        with conn.cursor() as cursor:
            ...
            st.success(f"✅ Upload history saved with ID: {last_id}")
            ...
            if saved_record:
                st.write("📋 Verified record in database:")
        ...
        if conn:
            conn.rollback()
        st.error(f"❌ Failed to log upload history: {e}")
        import traceback
        st.code(traceback.format_exc())
        return False
...
def save_transactions_to_db_with_logging(df, user_id, filename, batch_size=1000):
    """
    Save transactions with upload history logging - FIXED VERSION
    
    This version ensures upload history is ALWAYS logged, even on errors
    """
    import traceback
    ...
    st.info(f"📝 Starting upload logging for: {filename}")
    ...
    try:
        ...
        st.info("💾 Saving to transactions table...")
        inserted = save_transactions_to_db(df, user_id, batch_size)
        
        if inserted > 0:
            status = 'success'
            st.success(f"✅ Successfully saved {inserted:,} rows!")
        else:
            status = 'failed'
            error_msg = "No rows inserted - check data validation"
            st.warning("⚠️ No rows were inserted")
        ...
        # ✅ CRITICAL: ALWAYS log upload history, even on failure
        st.info("📝 Logging upload history...")
        ...
        log_success = log_upload_history(
            ...
            total_rows=total_rows if inserted > 0 else 0,
            status=status,
            error_msg=error_msg
        )
        ...
        if log_success:
            ...
            st.error("❌ Failed to log upload history - check database connection")
        ...
        with col3:
            st.metric("History Logged", "✅ Yes" if log_success else "❌ No")
