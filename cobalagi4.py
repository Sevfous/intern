...
def get_forecast_history(user_id: int, limit=20):
    """Get forecast history for a specific user with better formatting"""
    if user_id is None:
        return None
    ...
    try:
        ...
        # Query yang diperbaiki dengan grouping berdasarkan created_at
        query = """
            SELECT 
                model_type,
                DATE(created_at) as forecast_created_date,
                ...
                MAX(created_at) as created_timestamp
            FROM forecasts 
            WHERE user_id = %s 
        ...
        st.write("🔍 Debug - Kolom yang ada:", df.columns.tolist())
        
        # Format tanggal dengan error handling
        date_columns = {
            'forecast_created_date': 'Created Date',
        ...
        for old_col, new_col in date_columns.items():
            if old_col in df.columns:
                try:
                    ...
                    st.warning(f"⚠️ Error formatting {old_col}: {e}")
                    # If error, just convert to string
                    df[old_col] = df[old_col].astype(str)
        
        # Format angka dengan error handling
        numeric_columns = ['avg_forecast', 'min_forecast', 'max_forecast', 'total_forecast']
        for col in numeric_columns:
            if col in df.columns:
                try:
                    df[col] = df[col].apply(lambda x: f"{float(x):,.2f}" if pd.notna(x) else "0.00")
                except Exception as e:
                    st.warning(f"⚠️ Error formatting {col}: {e}")
        ...
        rename_dict = {old: new for old, new in column_rename.items() if old in df.columns}
        df = df.rename(columns=rename_dict)
        
        # Drop created_timestamp karena tidak perlu ditampilkan
        if 'created_timestamp' in df.columns:
            df = df.drop('created_timestamp', axis=1)
...
def get_forecast_details(user_id: int, created_date: str, model_type: str):
    """Get detailed forecast data for a specific forecast session"""
    if user_id is None:
        return None
    ...
    try:
        ...
        df = pd.read_sql(query, conn, params=(user_id, created_date, model_type))
...
def get_latest_forecast_summary(user_id: int):
    """Get summary of the most recent forecast"""
    if user_id is None:
        return None
...
def dashboard_page():
    ...
    # Display Forecast History if button clicked
    if st.session_state.get('show_forecast_history', False):
        ...
        if latest_forecast:
            ...
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("📊 Forecast Days", safe_metric_value(latest_forecast['forecast_days']))
            with col2:
                st.metric("📈 Avg Value", f"{latest_forecast['avg_value']:,.2f}")
            with col3:
                st.metric("💰 Total Value", f"{latest_forecast['total_value']:,.2f}")
            with col4:
                created = latest_forecast['created_at']
                st.metric("📅 Created", created.strftime('%Y-%m-%d') if created else 'N/A')
        ...
        if forecast_history is not None and len(forecast_history) > 0:
            ...
            # Option to view details of a specific forecast
            with st.expander("🔍 View Detailed Forecast Data"):
                if len(forecast_history) > 0:
                    # Get unique forecast sessions
                    forecast_sessions = []
                    for idx, row in forecast_history.iterrows():
                        session_label = f"{row['Model']} - {row['Created Date']} ({row['Days']} days)"
                    ...
                    selected_session = st.selectbox(
                        ...
                        options=range(len(forecast_sessions)),
                        format_func=lambda x: forecast_sessions[x]['label']
                    )
                    
                    if st.button("📊 Load Detailed Data", type="secondary"):
                        session = forecast_sessions[selected_session]
                        ...
                        if detailed_data is not None and len(detailed_data) > 0:
                            ...
                            ax.plot(
                                detailed_data['forecast_date'], 
                                detailed_data['forecast_value'],
                                marker='o',
                                linewidth=2,
                                markersize=4
                            )
                            ax.set_xlabel('Date', fontweight='bold')
                            ax.set_ylabel('Forecast Value', fontweight='bold')
                            ax.set_title(f"Forecast: {session['model']} - {session['date']}", fontweight='bold')
                            ...
                            # Show data table
                            st.dataframe(detailed_data, use_container_width=True, height=300)
                            ...
                            st.warning("⚠️ No detailed data found for this forecast session.")
            
            # Close button
            if st.button("❌ Close Forecast History", type="secondary"):
                st.session_state['show_forecast_history'] = False
                st.rerun()
        else:
            st.info("ℹ️ No forecast history found. Generate your first forecast in the 'Generate Forecast' menu!")
