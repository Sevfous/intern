...
def get_user_statistics(user_id: int):
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("SELECT * FROM v_user_statistics WHERE id = %s", (user_id,))
            result = cursor.fetchone()
            ...
            if result:
                return {
                    ...
                    'latest_upload': result['latest_upload_date']
                }
            else:
                return {'total_transactions': 0, 'total_forecasts': 0, 'latest_upload': None}
    except Exception as e:
...
def get_all_users_admin():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("SELECT * FROM v_admin_user_management")
            results = cursor.fetchall()
...
def get_latest_uploads_admin():
    """Get latest CSV uploads per user (admin only)"""
    conn = get_connection()
    try:
        with conn.cursor() as cursor:
            query = """
...
def log_upload_history(user_id, username, filename, file_size_mb, total_rows, status='success', error_msg=None):
    """Log CSV upload to history table"""
    conn = get_connection()
    try:
        with conn.cursor() as cursor:
            query = """
        ...
        st.error(f"Failed to log upload: {e}")
        return False
    finally:
        conn.close()
...
def save_transactions_to_db_with_logging(df, user_id, filename, batch_size=1000):
    """Save transactions with upload history logging"""
...
def dashboard_page():
    ...
    if latest_forecast:
        ...
        if st.button("📈 Generate First Forecast", type="primary", use_container_width=True):
            st.session_state['navigate_to'] = '📈 Generate Forecast'
            st.rerun()
...
def admin_dashboard_page():
    ...
    with tab2:
        ...
        if len(uploads_df) > 0:
            ...
            display_uploads['file_size_mb'] = display_uploads['file_size_mb'].apply(lambda x: f"{x:.2f} MB")
            display_uploads['total_rows'] = display_uploads['total_rows'].apply(lambda x: f"{x:,}")
    ...
    with tab3:
        ...
        if len(users_df) > 0:
            ...
            if latest_upload:
                st.subheader("📤 Latest Upload Details")
                col1, col2, col3 = st.columns(3)
                with col1:
                    st.info(f"**File:** {latest_upload['filename']}")
                with col2:
                    st.info(f"**Size:** {latest_upload['file_size_mb']:.2f} MB")
                with col3:
                    st.info(f"**Rows:** {latest_upload['total_rows']:,}")
        else:
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        with st.sidebar:
            ...
            # ✅ MODIFIED: Admin section dengan mode switcher
            if st.session_state.is_admin:
                st.success("🔐 **ADMIN ACCESS**")
                st.markdown("---")
                
                # ✅ BARU: Mode switcher untuk admin
                st.subheader("🔄 View Mode")
                view_mode = st.radio(
                    "Select Mode:",
                    options=["User Mode", "Admin Management"],
                    key="admin_view_mode_radio",
                    help="User Mode: Access all user features\nAdmin Management: Manage users"
                )
                st.session_state.admin_view_mode = view_mode
                
                # ✅ BARU: Tampilkan statistik admin di User Mode
                ...
                # Regular user section (tidak berubah)
            ...
            # ✅ MODIFIED: Menu untuk user DAN admin (jika User Mode)
            if not st.session_state.is_admin or st.session_state.admin_view_mode == "User Mode":
                ...
                # Admin Management mode tidak perlu menu (akan tampil admin panel)
                menu = None
            ...
            if st.button("🚪 Logout", use_container_width=True, type="primary"):
                ...
                st.session_state.cleaning_done = False
                st.session_state.db_after_clean = 0
                st.session_state.admin_view_mode = "User Mode"  # ✅ TAMBAHAN: Reset mode
                gc.collect()
                st.rerun()
        
        # ✅ MODIFIED: Routing dengan conditional untuk admin mode
        if st.session_state.is_admin and st.session_state.admin_view_mode == "Admin Management":
            # Admin di Management Mode
            admin_dashboard_page()
        elif menu:  # ✅ TAMBAHAN: Check menu exists
            # User routing ATAU Admin di User Mode
            if menu == "🏠 Dashboard":
                dashboard_page()
