        ...
        with conn.cursor() as cursor:
            query = """
                SELECT 
                    ...
                    COUNT(DISTINCT DATE(f.created_at)) as total_forecasts
                FROM users u
...
def cleaning_data_page():
    ...
    if st.session_state.csv_data is not None:
        ...
        if st.session_state.cleaning_done and st.session_state.cleaned_data is not None:
            ...
            with col3:
                if st.button("🔄 Clean Again", use_container_width=True):
                    st.session_state.cleaning_done = False
                    st.session_state.cleaned_data = None
                    st.rerun()
...
def main():
    ...
    # ✅ Tambahkan is_admin ke session state init
    if 'is_admin' not in st.session_state:
        st.session_state.is_admin = False
    ...
    if not st.session_state.logged_in:
        ...
        if st.session_state.is_admin:
            ...
            # User menu routing (sama seperti sebelumnya)
            if menu == "🏠 Dashboard":
                dashboard_page()
