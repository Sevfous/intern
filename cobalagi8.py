        ...
        with conn.cursor() as cursor:
            query = """
                SELECT 
                    ...
                    (
                        SELECT COUNT(DISTINCT CONCAT(model_type, '_', DATE(created_at)))
                        FROM forecasts
                        WHERE user_id = u.id
                    ) as total_forecasts
                FROM users u
    ...
    if not st.session_state.logged_in:
        ...
        with st.sidebar:
            ...
            if st.session_state.is_admin:
                ...
                # USER STATS (hanya untuk user biasa)
                user_stats = get_user_statistics(st.session_state.user_id)
                st.markdown("---")
        ...
        # ✅ Route: Admin langsung ke dashboard, User pakai menu
        if st.session_state.is_admin:
