...
def save_transactions_to_db(df: pd.DataFrame, user_id: int, batch_size=1000):
    ...
    Optimized for large datasets - FIXED VERSION
    """
...
def get_forecast_history(user_id: int, limit=20):
    ...
    Returns summary table with aggregated data - FIXED VERSION
    """
    if user_id is None:
        return None
...
def get_forecast_details(user_id: int, session_date: str, model_type: str):
    """
    Get detailed daily forecast values - FIXED VERSION
    """
    if user_id is None:
        return None
...
def get_latest_forecast_summary(user_id: int):
    """
    Get summary of the most recent forecast session - FIXED VERSION
    """
    if user_id is None:
        return None
...
def dashboard_page():
    ...
    # ========== FORECAST HISTORY SECTION WITH ENHANCED VISUALIZATION ==========
    if st.session_state.get('show_forecast_history', False):
        st.markdown("---")
        st.header("📜 Forecast History & Visualization")
        
        # Get latest forecast summary
        latest_forecast = get_latest_forecast_summary(st.session_state.user_id)
        
        if latest_forecast:
            st.success(f"🎯 Latest Forecast: **{latest_forecast['model_type']}** Model")
            
            # === LOAD LATEST FORECAST DETAILS ===
            ...
            if latest_details is not None and len(latest_details) > 0:
                ...
                with col3:
                    min_val = latest_details['Forecast Value'].min()
                    st.metric("📉 Min", f"{float(min_val):,.0f}")
                with col4:
                    max_val = latest_details['Forecast Value'].max()
                    st.metric("📈 Max", f"{float(max_val):,.0f}")
                with col5:
                ...
                st.subheader("📊 Latest Forecast Visualization")
                ...
                try:
                    ...
                    # Prepare data
                    dates = latest_details['Date'].values
                    ...
                    ax1.axhline(y=mean_val, color='#26DE81', linestyle=':', 
                               linewidth=2, alpha=0.7, label=f'Mean: {mean_val:,.0f}')
                    
                    ax1.set_xlabel('Date', fontsize=11, fontweight='bold')
                    ax1.set_ylabel('Forecast Value', fontsize=11, fontweight='bold')
                    ax1.set_title(f'Latest Forecast - {latest_forecast["model_type"]} Model', 
                    ...
                    colors = ['#26DE81' if v >= mean_val else '#EE5A6F' for v in values]
                    ax2.bar(dates, values, color=colors, alpha=0.7, 
                           edgecolor='black', linewidth=0.5)
                    ax2.axhline(y=mean_val, color='#2E86DE', linestyle='--', 
                               linewidth=2, label=f'Mean: {mean_val:,.0f}')
                    
                    ax2.set_xlabel('Date', fontsize=11, fontweight='bold')
                    ax2.set_ylabel('Forecast Value', fontsize=11, fontweight='bold')
                    ax2.set_title('Daily Forecast Distribution', fontsize=13, 
                ...
                display_df = latest_details.copy()
                display_df['Date'] = pd.to_datetime(display_df['Date']).dt.strftime('%Y-%m-%d')
                display_df['Forecast Value'] = display_df['Forecast Value'].apply(
                ...
                with col1:
                    csv_latest = latest_details.to_csv(index=False)
                    st.download_button(
                        label="⬇️ Download Latest Forecast (CSV)",
                        data=csv_latest,
                        file_name=f"latest_forecast_{latest_forecast['model_type']}_{session_date}.csv",
                        mime="text/csv",
                ...
                with col2:
                    import json
                    summary_json = {
                        'model': latest_forecast['model_type'],
                        'created_date': session_date,
                        'days': int(latest_forecast['forecast_days']),
                        'statistics': {
                            'mean': float(mean_val),
                            'min': float(min_val),
                            'max': float(max_val),
                            'std': float(std_dev),
                            'total': float(latest_forecast['total_value'])
                        }
                    }
                    st.download_button(
                        label="⬇️ Download Summary (JSON)",
                        data=json.dumps(summary_json, indent=2),
                        file_name=f"forecast_summary_{session_date}.json",
                        mime="application/json",
                        use_container_width=True
                    )
            
            else:
                st.warning("⚠️ No detailed forecast data found for the latest session.")
                st.info("💡 Try generating a new forecast in the 'Generate Forecast' menu.")
            
            st.markdown("---")
        
        else:
            st.info("ℹ️ No forecast history found.")
            st.write("Generate your first forecast in the **'📈 Generate Forecast'** menu!")
        
        # === ALL FORECAST HISTORY TABLE ===
        ...
        if forecast_history is not None and len(forecast_history) > 0:
            st.subheader("📋 All Forecast Sessions")
            st.info(f"ℹ️ Showing {len(forecast_history)} most recent sessions")
            
            st.dataframe(forecast_history, use_container_width=True, height=350, hide_index=True)
            
            # === VIEW OTHER FORECAST DETAILS ===
            st.markdown("---")
            with st.expander("🔍 View Other Forecast Details", expanded=False):
                sessions = []
                for idx, row in forecast_history.iterrows():
                    label = f"{row['Model']} - {row['Session Date']} ({row['Days']} days)"
                    sessions.append({
                        'label': label,
                ...
                selected_idx = st.selectbox(
                    "📊 Select Forecast Session:",
                    options=range(len(sessions)),
                    format_func=lambda x: sessions[x]['label']
                )
                
                if st.button("📊 Load Selected Forecast", type="secondary", use_container_width=True):
                    session = sessions[selected_idx]
                    
                    with st.spinner("Loading forecast details..."):
                        detailed_data = get_forecast_details(
                            st.session_state.user_id,
                            session['date'],
                            session['model']
                        )
                    ...
                    if detailed_data is not None and len(detailed_data) > 0:
                        st.success(f"✅ Loaded {len(detailed_data)} forecast points")
                        
                        # Statistics
                        col1, col2, col3, col4 = st.columns(4)
                        with col1:
                            st.metric("Min", f"{detailed_data['Forecast Value'].min():,.2f}")
                        with col2:
                            st.metric("Avg", f"{detailed_data['Forecast Value'].mean():,.2f}")
                        with col3:
                            st.metric("Max", f"{detailed_data['Forecast Value'].max():,.2f}")
                        with col4:
                            st.metric("Total", f"{detailed_data['Forecast Value'].sum():,.2f}")
                        
                        # Chart
                        st.subheader("📈 Forecast Chart")
                        fig, ax = plt.subplots(figsize=(14, 5))
                        
                        ax.plot(detailed_data['Date'], detailed_data['Forecast Value'],
                               marker='o', linewidth=2.5, markersize=5, color='#1f77b4',
                               label='Forecast')
                        
                        mean_line = detailed_data['Forecast Value'].mean()
                        ax.axhline(y=mean_line, color='red', linestyle='--', linewidth=2,
                                  alpha=0.7, label=f'Mean: {mean_line:,.0f}')
                        
                        ax.set_xlabel('Date', fontsize=12, fontweight='bold')
                        ax.set_ylabel('Forecast Value', fontsize=12, fontweight='bold')
                        ax.set_title(f"Forecast Details: {session['model']} - {session['date']}", 
                        ...
                        st.download_button(
                            ...
                            data=csv_data,
                            file_name=f"forecast_{session['model']}_{session['date']}.csv",
                            mime="text/csv",
                            use_container_width=True
                        )
                    else:
                        st.warning("⚠️ No detailed data found for this session.")
...
def add_data_page():
    ...
    with tab1:
        ...
        if uploaded_file is not None:
            ...
            # ✅ FIX: Simplified chunk size logic
            ...
            if use_chunked:
                ...
                # ✅ FIX: Use selectbox instead of number_input
                chunk_options = {
                    "10,000 rows": 10000,
                    "50,000 rows": 50000,
                        ...
                        if df is not None and len(df) > 0:
                            ...
                            if auto_save_to_db:
                                with st.spinner("💾 Menyimpan ke database..."):
                                    # ✅ FIX: Pass explicit batch_size
                                    inserted = save_transactions_to_db(
                                        df, 
                                        st.session_state.user_id,
                                        batch_size=1000  # Explicit value
                                    )
                                    if inserted > 0:
...
def generate_forecast_page():
    ...
    if st.button("🚀 Generate Forecast", type="primary", use_container_width=True):
        with st.spinner(f"Generating {model_choice} forecast for {forecast_days} days..."):
            try:
                # ✅ FIXED: Proper time series preparation
                df_ts = data_source.copy()
                
                # ✅ FIX: Force convert to datetime (handle CategoricalIndex issue)
                if date_col in df_ts.columns:
                    ...
                    st.error(f"❌ Kolom {date_col} tidak ditemukan")
                    return
                ...
                if not isinstance(df_ts.index, pd.DatetimeIndex):
                    st.error("❌ Index bukan DatetimeIndex. Konversi gagal.")
                    st.write(f"Current index type: {type(df_ts.index)}")
                    return
