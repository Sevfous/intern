...
def save_transactions_to_db(df: pd.DataFrame, user_id: int):
    ...
    all_cols = ['user_id'] + available_cols
    ...
    for _, row in df.iterrows():
        row_vals = [user_id]  # Start with user_id
        skip_row = False
...
def load_transactions_from_db(user_id: int, limit=MAX_ROWS_IN_MEMORY):
    """Load transactions with LIMIT - FIX for MessageSizeError"""
    if user_id is None:
        return None
    ...
    try:
        ...
        if len(df) > 0:
            ...
            if len(df) >= limit:
                st.info(f"📊 Loaded most recent {limit:,} transactions. Total may be more in database.")
            else:
                st.success(f"✅ Loaded all {len(df):,} transactions from database")
