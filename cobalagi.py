...
def save_transactions_to_db(df: pd.DataFrame, username: str):
    """Save transactions to database (only matching columns)"""
    if df is None or len(df) == 0:
        return 0
    
    db_columns = ['order_date', 'Final_Price', 'qty', 'price']
    available_cols = [col for col in db_columns if col in df.columns]
    
    if len(available_cols) == 0:
        st.error("❌ Tidak ada kolom yang cocok dengan database. Pastikan CSV memiliki kolom: order_date, Final_Price")
        return 0
    
    if 'order_date' not in available_cols:
        st.error("❌ Kolom 'order_date' wajib ada di CSV!")
        return 0
    
    if 'Final_Price' not in available_cols:
        st.error("❌ Kolom 'Final_Price' wajib ada di CSV!")
        return 0
    
    placeholders = ", ".join(["%s"] * len(available_cols))
    cols_sql = ", ".join([f"`{c}`" for c in available_cols])
    ...
    for _, row in df.iterrows():
        ...
        for c in available_cols:
            ...
            if c in ['order_date', 'Final_Price'] and pd.isna(v):
                skipped += 1
                skip_row = True
                break
    ...
    if len(values) == 0:
        st.error("❌ Tidak ada data valid untuk disimpan. Pastikan order_date dan Final_Price tidak kosong.")
        return 0
    ...
    try:
        ...
        if skipped > 0:
            st.warning(f"⚠️ {skipped:,} baris dilewati karena order_date atau Final_Price kosong")
        
    except Exception as e:
        conn.rollback()
        st.error(f"❌ Failed to save transactions to DB: {e}")
...
def save_forecasts_to_db(username: str, model: str, dates, values):
    if username is None:
        st.error("User tidak ditemukan. Login terlebih dahulu untuk menyimpan forecast.")
        return 0
    insert_sql = "INSERT INTO forecasts (username, model_type, forecast_date, forecast_value) VALUES (%s, %s, %s, %s)"
    payload = []
    for d, v in zip(dates, values):
        if isinstance(d, (pd.Timestamp, datetime)):
            ...
            d_val = str(d)
        payload.append((username, model, d_val, float(v)))
    conn = get_connection()
    inserted = 0
    try:
        with conn.cursor() as cur:
    ...
    with tab3:
        ...
        if st.button("🔄 Change Password", key="change_pass_btn", type="primary", use_container_width=True):
            ...
            if not change_username or not old_password or not new_password or not confirm_new_password:
                ...
                try:
                    with conn.cursor() as cursor:
                        # Cek apakah username dan password lama benar
    ...
    with tab1:
        ...
        if uploaded_file is not None:
            ...
            auto_save_to_db = st.checkbox(
                ...
                value=True,
                help="Data akan langsung disimpan ke tabel 'transactions' setelah berhasil di-load"
            )
                        ...
                        if df is not None:
                            ...
                            if auto_save_to_db:
                                with st.spinner("💾 Menyimpan ke database..."):
                                    inserted = save_transactions_to_db(df, st.session_state.get('username'))
                                    if inserted > 0:
                                        st.success(f"✅ Berhasil menyimpan {inserted:,} baris ke tabel `transactions`")
                                    else:
...
def generate_forecast_page():
    ...
    if st.session_state.forecast_result:
        ...
        ax.plot(recent_train.index, recent_train.values, label='Historical Data', linewidth=2.5, alpha=0.8)
        ax.plot(result['test_data'].index, result['test_data'].values, label='Actual (Test Period)', linewidth=2.5, marker='o', markersize=4)
        ax.plot(result['test_data'].index, result['test_forecast'], label='Forecast (Test Period)', linewidth=2, linestyle='--', marker='s', markersize=3)
        ...
        ax.plot(future_dates_with_connection, future_values_with_connection, label=f'Future Forecast ({result["forecast_days"]} days)', linewidth=2.5, linestyle='--', marker='o', markersize=4)
        ...
        ax.text(result['train_data'].index[-1], ax.get_ylim()[1]*0.95, 'Today', ha='center', fontsize=9, color='gray', fontweight='bold')
        plt.tight_layout()
        st.pyplot(fig)
        ...
        if st.button("💾 Save Forecast to Database"):
            inserted = save_forecasts_to_db(st.session_state.get('username'), result['model'], result['future_dates'], result['future_forecast'])
            if inserted:
                st.success(f"✅ Berhasil menyimpan {inserted:,} baris ke tabel `forecasts`")
            else:
                st.error("❌ Gagal menyimpan hasil forecast ke database.")
