...
def get_user_statistics(user_id: int):
    """
    Get statistics for a specific user - FIXED VERSION
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT COUNT(DISTINCT DATE(created_at)) as count FROM forecasts WHERE user_id = %s LIMIT 1", 
                (user_id,)
            )
            result = cursor.fetchone()
...
def get_all_users_admin():
    """
    Get all users with their passwords (admin only) - FIXED VERSION
                ...
                for idx, row in df.iterrows():
                    cursor.execute(
                        """SELECT COUNT(DISTINCT DATE(created_at)) as count 
                           FROM forecasts WHERE user_id = %s LIMIT 1""",
                        (row['id'],)
                    )
        ...
        try:
            ...
            with tab2:
                ...
                if len(uploads_df) > 0:
                    ...
                    st.dataframe(display_uploads, use_container_width=True, height=500)
                else:
